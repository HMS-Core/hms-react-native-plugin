...
public class HMSTextImageSuperResolution extends HMSBase {
    ...
    public void analyzeFrame(boolean isStop, ReadableMap frameConfiguration, final Promise promise) {
        ...
        MLTextImageSuperResolutionAnalyzer imageSuperResolutionAnalyzer = MLTextImageSuperResolutionAnalyzerFactory.getInstance().getTextImageSuperResolutionAnalyzer();
        HMSBackgroundTasks.getInstance().saveTextImageSuperResolutionImages(getContext(), imageSuperResolutionAnalyzer.analyseFrame(frame))
    ...
    public void asyncAnalyzeFrame(boolean isStop, ReadableMap frameConfiguration, final Promise promise) {
        ...
        textImageSuperResolutionAnalyzer.asyncAnalyseFrame(frame)
                .addOnSuccessListener(textImageSuperResolution -> {
                    if (isStop)
                        textImageSuperResolutionAnalyzer.stop();
                    HMSBackgroundTasks.getInstance().saveImageAndGetUri(getContext(), textImageSuperResolution.getBitmap())
