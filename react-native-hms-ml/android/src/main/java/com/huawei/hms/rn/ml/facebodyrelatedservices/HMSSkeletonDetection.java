...
public class HMSSkeletonDetection extends HMSBase {
     ...
     */
    @ReactMethod
    public void asyncAnalyzeFrame(boolean isStop, int analyzeType, ReadableMap frameConfiguration, final Promise promise) {
        ...
        MLSkeletonAnalyzer skeletonAnalyzer = HMSObjectCreator.getInstance().createSkeletonAnalyzer(analyzeType);
        skeletonAnalyzer.asyncAnalyseFrame(frame)
                .addOnSuccessListener(mlSkeletons -> {
                    if (isStop)
                        stopAnalyzer(skeletonAnalyzer);
                    handleResult("asyncAnalyzeFrame", HMSResultCreator.getInstance().getSkeletonAsyncResults(mlSkeletons), promise);
                })
     ...
     */
    @ReactMethod
    public void calculateSimilarity(boolean isStop, int analyzeType, ReadableArray dataSet1, ReadableArray dataSet2, final Promise promise) {
