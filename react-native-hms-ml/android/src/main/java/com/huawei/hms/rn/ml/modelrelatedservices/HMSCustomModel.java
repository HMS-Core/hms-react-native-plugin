...
public class HMSCustomModel extends HMSBase {
     ...
     */
    @ReactMethod
    public void exec(boolean isRemote, ReadableMap modelInputOutputSettings, ReadableMap modelInputConfiguration, ReadableMap modelConfig, final Promise promise) {
        startMethodExecTimer("exec");

        MLModelInputOutputSettings settings = HMSObjectCreator.getInstance().createCustomModelInputOutputSetting(modelInputOutputSettings);
        MLModelInputs modelInputs = HMSObjectCreator.getInstance().createCustomModelInputs(modelInputConfiguration, getContext());
        MLModelExecutorSettings modelExecutorSetting = HMSObjectCreator.getInstance().createCustomModelExecutorSettings(isRemote, modelConfig);
        ...
        try {
            MLModelExecutor.getInstance(modelExecutorSetting).exec(modelInputs, settings)
                    .addOnSuccessListener(mlModelOutputs -> handleResult("exec", HMSResultCreator.getInstance().customModelResult(mlModelOutputs), promise))
    ...
    public void getOutputIndex(boolean isRemote, String channelName, ReadableMap modelConfig, final Promise promise) {
        ...
        try {
            MLModelExecutor.getInstance(modelExecutorSetting).getOutputIndex(channelName)
                    .addOnSuccessListener(integer -> handleResult("getOutputIndex", HMSResultCreator.getInstance().integerResult(integer), promise))
