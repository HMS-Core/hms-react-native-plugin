...
public class HmsActivityRecordsController extends ReactContextBaseJavaModule implements BaseProtocol.Event {
    ...
    public void beginActivityRecord(final ReadableMap readableMap, final Promise promise) {
        ...
        ActivityRecord activityRecord = ActivityRecordsUtils.INSTANCE.toActivityRecord(readableMap, reactContext, promise);
     ...
     * @param dataCollectorReadableMap  ReadableMap instance that refers to {@link DataCollector} instance.
     * @param sampleSetMapArr           ReadableMap instance that refers to {@link SampleSet} instance.
     * @param promise                   In the success scenario, {@link Void} instance is returned with {@code isSuccess: true} params , or Exception is returned in the failure scenario.
     */
    @ReactMethod
    public void addActivityRecord(final ReadableMap activityRecordReadableMap, final ReadableMap dataCollectorReadableMap, final ReadableArray sampleSetMapArr, final Promise promise) {
        ...
        DataCollector dataCollector = Utils.INSTANCE.toDataCollector(dataCollectorReadableMap, reactContext);

        // Build the time range of the request object: start time and end time
        // Build the activity record request object
        ActivityRecord activityRecord = ActivityRecordsUtils.INSTANCE.toActivityRecord(activityRecordReadableMap, reactContext, promise);

        // Build the sampling sampleSet based on the dataCollector
        // Build the data type and add it to the sampling dataSet
        SampleSet sampleSet = Utils.INSTANCE.toSampleSet(sampleSetMapArr, dataCollector, promise);
     ...
     */
    @ReactMethod
    public void addMultipleActivityRecords(final ReadableMap activityRecordReadableMap, final ReadableArray sampleSetMapArr, final Promise promise) {
        ...
        activityRecordsViewModel.addMultipleActivityRecord(this.activityRecordsController, activityRecord, sampleSetList,
                new VoidResultHelper(promise, logger, logName));
    }
     ...
     */
    @ReactMethod
    public void getActivityRecord(final ReadableMap dataTypeMap, final ReadableMap dateMap, final @Nullable String activityRecordId, final @Nullable String activityRecordName, final Promise promise) {
        ...
        ActivityRecordReadOptions readRequest = ActivityRecordsUtils.INSTANCE.toActivityRecordReadOptions(dataTypeMap, dateMap, activityRecordId, activityRecordName, promise);

        // Get the requested ActivityRecords
        activityRecordsViewModel.getActivityRecord(this.activityRecordsController, readRequest, new ResultHelper<>(ActivityRecordReply.class, promise, logger, logName));
     ...
     * @param dataTypeMap        ReadableMap instance that refers to {@link DataType} instance.
     * @param dateMap            ReadableMap instance that refers to startTime, endTime and timeUnit params.
     * @param activityRecordId   String value of activityRecordId that can either be decleared or be null.
     * @param activityRecordName String value of activityRecordName that can either be decleared or be null
     ...
     */
    @ReactMethod
    public void deleteActivityRecord(final ReadableMap dataTypeMap, final ReadableMap dateMap, final @Nullable String activityRecordId, final @Nullable String activityRecordName, final Promise promise) {
        ...
        // Delete the requested ActivityRecords
        activityRecordsViewModel.deleteActivityRecord(this.activityRecordsController, dateMap, this.dataController, readRequest, new ResultHelper<>(List.class, promise, logger, logName));
    }
    ...
    public void sendEvent(ReactContext reactContext, String eventName, @Nullable WritableMap params) {
        reactContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class).emit("addActivityRecordsMonitor", params);
    }
