...
public class HmsHealthRecordController extends BaseController implements BaseProtocol.Event {
     ...
     */
    @ReactMethod
    public void addHealthRecord(ReadableArray healthRecordBuilder,final ReadableArray dataCollectorArray, final ReadableArray sampleSet, final Promise promise){
        ...
        List<Object> sampleSetList = toArrayList(sampleSet);
        ...
        HealthRecord healthRecord = HealthRecordControllerUtils.INSTANCE.toHealthRecord(healthRecordBuilder,sampleSetList1,samplePointList,dataCollector2);

        healthRecordViewModel.addHealthRecord(healthRecordController,healthRecord,new ResultHelper(String.class,promise,logger,logName));
    }
     ...
     */
    @ReactMethod
    public void updateHealthRecord(ReadableArray healthRecordBuilder,final ReadableArray dataCollectorArray, final ReadableArray sampleSet, final Promise promise){
        ...
        DataCollector dataCollector2 = Utils.INSTANCE.toDataCollector(dataCollectorArray.getMap(2),reactContext);
        samplePointList.add(samplePoint1);
        HealthRecord updateHealthRecord = HealthRecordControllerUtils.INSTANCE.toHealthRecord(healthRecordBuilder,sampleSetList1,samplePointList,dataCollector2);

        healthRecordViewModel.updateHealthRecord(healthRecordController,updateHealthRecord,new VoidResultHelper(promise,logger,updateLogName));
    }
    ...
    public void getHealthRecord(final ReadableMap dataType, final ReadableMap dateMap, final Promise promise) {
        ...
        HealthRecordReadOptions healthRecordReadOptions = HealthRecordControllerUtils.INSTANCE.toReadHealthRecordOptions(dataType, dateMap, promise);

        healthRecordViewModel.getHealthRecord(healthRecordController, healthRecordReadOptions, new ResultHelper<>(HealthRecordReply.class, promise, logger, logName));
    }
